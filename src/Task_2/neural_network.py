...
class NeuralNetwork:
    ...
    def build_network(self):
        ...
        for i in range(1, len(self.layers)):
            ...
            self.layers[i].w = tn.rvs(
                (self.layers[i - 1].n, self.layers[i].n)
            )  # initialise weights matrices for layers
            self.layers[i].b = np.zeros(
                (1, self.layers[i].n)
            )  # initialise biases for layers
    ...
    def d_loss(self, y_pred, y_test):
        ...
        if self.type_loss == "mse":
            ...
            return (
                y_pred - y_test
            )  # here y_pred is the probability given by softmax layer
    ...
    def fit_batch(self, X, y, toll):
        ...
        for epoch in range(self.max_epochs):
            ...
            dL = self.d_loss(
                io_layer, y
            )  # this will be the first error to be transferred backwards

            io_error = (
                dL  # initialise the first gradient with the derivative of the loss
            )

            for i in range(
                len(self.layers) - 1, 0, -1
            ):  # from last layer to the first layer
            ...
            if epoch > 3:
                if (
                    abs(L_history[-1] - L) <= toll
                    and abs(L_history[-2] - L) <= toll
                    and abs(L_history[-3] - L) <= toll
                ):
                    print("Stop!")
                    break
    ...
    def dropout_matrices(self):
        ...
        indices = range(
            self.layers[-1].n
        )  # initialiase indices - no dropout in output layer
    ...
    def fit_mini_batch(self, X, y, toll, n_mini_batches=32):
        ...
        batches = [
            range(x, min(x + batch_size, len(X))) for x in range(0, len(X), batch_size)
        ]
        ...
        for epoch in range(self.max_epochs):
            ...
            if epoch > 3:
                if (
                    abs(self.L_history[-2] - self.L_history[-1]) <= toll
                    and abs(self.L_history[-3] - self.L_history[-1]) <= toll
                    and abs(self.L_history[-4] - self.L_history[-1]) <= toll
                ):
                    print("Stop!")
                    break
