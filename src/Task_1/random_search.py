...
class RandomSearch:
    ...
    def compute_path(self):
        ...
        while not (
            (current_node[0] == (self.h - 1)) and (current_node[1] == (self.w - 1))
        ):

            next_node = self.random_step(
                current_node
            )  # select a random step from the current node
            ...
            ):  # when the maximum number of steps has been reached, the method stops
    ...
    def random_step(self, current_node):
        ...
        if random_direction == 0:  # step down
            if (
                i == self.h - 1
            ):  # when the agent is the the lower border, skip the next step
                pass
            else:
                next_node = (i + 1, j)
            ...
            if (
                j == self.w - 1
            ):  # when the agent is the the right border, skip the next step
                pass
            else:
                next_node = (i, j + 1)
